...
/*
CVarBool vk_debug("vk_debug", false);
CVarInt vk_device("vk_device", 0);

CCMD vk_listdevices("vk_listdevices", [](std::string args)
{
	for (size_t i = 0; i < supportedDevices.size(); i++)
	{
		printLog(notifyColor, "#%1 - %2\n", (int)i, supportedDevices[i].device->properties.deviceName);
	}
});
*/

VulkanDevice::VulkanDevice(Win32Window *window) : window(window)
{
	try
	{
		initVolk();
...
void VulkanDevice::createSurface()
{
	...
	windowCreateInfo.hwnd = (HWND)window->window;
