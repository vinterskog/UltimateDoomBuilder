...
namespace CodeImp.DoomBuilder.DBXLua
{
    ...
    public class LuaUI
    {
        ...
        public static Table AskForParametersNoClear()
        {
            ...
            {
				General.ErrorLogger.Add(ErrorType.Error, "Inconsistent parameter counts in Lua script!\n datatypes: ");
                foreach (DataType o in ScriptContext.context.ui_parameters.datatypes)
                {
					General.ErrorLogger.Add(ErrorType.Error, o.ToString());
                }

				General.ErrorLogger.Add(ErrorType.Error, "keys: ");
                foreach (string o in ScriptContext.context.ui_parameters.keys)
                {
                    if (o == null)
                    {
						General.ErrorLogger.Add(ErrorType.Error, "null");
                    }
                    else
                    {
						General.ErrorLogger.Add(ErrorType.Error, o);
                    }
                }

				General.ErrorLogger.Add(ErrorType.Error, "labels: ");
